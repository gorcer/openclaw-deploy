#!/bin/bash
# OpenClaw Client Deployment Script with Security & Proxy
# Скрипт для поднятия нового клиента в изолированном контейнере с xray
...
# Переменные окружения клиента (передаются как аргументы)
TELEGRAM_BOT_TOKEN="${TELEGRAM_BOT_TOKEN}"
MINIMAX_API_KEY="${MINIMAX_API_KEY}"
YANDEX_API_KEY="${YANDEX_API_KEY}"
YANDEX_FOLDER_ID="${YANDEX_FOLDER_ID}"
# Прокси (если нужно ходить через внешний прокси)
PROXY_OUTBOUND="${PROXY_OUTBOUND:-""}"

echo "=== OpenClaw Client Deployment (Secure + Proxy) ==="
...
# Создание директории для xray конфига
mkdir -p "${CLIENT_DIR}/xray"

# === XRAY CONFIG ===
# Создание конфига xray (VMess/VLess + SOCKS + HTTP)
...
# Добавляем outbound прокси если указан
...
services:
  xray:
    ...
    ports:
      ...
      - "127.0.0.1:10809:10809"   # HTTP (только localhost)
    networks:
      - openclaw_net
